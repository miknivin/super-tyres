...
interface ChecklistItems {
  ...
  steeringCentred: boolean;
  beforeAfterReporting: boolean;
}

const AlignmentChecklist: React.FC = () => {
  // Vehicle data
  const vehicleData = {
  ...
  const [checklistItems, setChecklistItems] = useState<ChecklistItems>({
    ...
    steeringCentred: false,
    beforeAfterReporting: false
  });
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center mb-6">
          ...
          <span className="text-teal-600 text-sm font-medium">Alignment Technician Checklist</span>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-4 mb-6">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  checklistItems.suspensionChecked ? 'bg-teal-600' : 'bg-gray-300'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    checklistItems.suspensionChecked ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => toggleItem('steeringCentred')}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  checklistItems.steeringCentred ? 'bg-teal-600' : 'bg-gray-300'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    checklistItems.steeringCentred ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-700 text-sm">Before/After Reporting Printed</span>
              <button
                onClick={() => toggleItem('beforeAfterReporting')}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  checklistItems.beforeAfterReporting ? 'bg-teal-600' : 'bg-gray-300'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    checklistItems.beforeAfterReporting ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
