// src/components/checklists/BalancingChecklist.tsx
...
interface BalancingChecklistItems {
...
interface BalancingChecklistProps {
  enquiryId: string;
}

const BalancingChecklist: React.FC<BalancingChecklistProps> = ({
  enquiryId,
}) => {
  const {
    formState,
    ...
    isEnquiryError,
    enquiry,
  } = useChecklistForm<BalancingChecklistItems>({
    enquiryId,
    checklistType: "balancing-technician-checklist",

    // Default / empty state when no data yet
    initialState: {
      wheelCleaned: false,
      weightsFixedSecurely: false,
      finalRecheckDone: false,
    },

    // Mapping: local state key → exact backend response field name
    fieldMapping: {
      wheelCleaned: "wheelCleaned",
      weightsFixedSecurely: "weightsFixedSecurely",
      finalRecheckDone: "finalRecheckDone",
    },
  ...
  if (isChecklistLoading || isEnquiryLoading) {
    return (
      <div className="min-h-screen bg-gray-50 p-4 flex items-center justify-center">
        <div className="text-gray-600">Loading balancing checklist...</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-4 flex items-center justify-center">
        <div className="text-red-600 text-center">
          Failed to load balancing checklist
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-4 mb-6">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                  formState.wheelCleaned ? "bg-teal-600" : "bg-gray-300"
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    formState.wheelCleaned ? "translate-x-6" : "translate-x-1"
                  }`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-700 text-sm">
                Weight Fixed Securely
              </span>
              <button
                type="button"
                onClick={() => toggleItem("weightsFixedSecurely")}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                  formState.weightsFixedSecurely ? "bg-teal-600" : "bg-gray-300"
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    formState.weightsFixedSecurely
                      ? "translate-x-6"
                      : "translate-x-1"
                  }`}
                />
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                  formState.finalRecheckDone ? "bg-teal-600" : "bg-gray-300"
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    formState.finalRecheckDone
                      ? "translate-x-6"
                      : "translate-x-1"
                  }`}
                />
