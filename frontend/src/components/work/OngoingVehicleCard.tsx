...
export default function OngoingVehicleCard({ vehicle }: Props) {
  const isAwaiting = vehicle.status === "awaiting";
  ...
  return (
    <>
      ...
      <div className="flex gap-3">
        ...
        >
          {isAwaiting ? "Awaiting approval" : "Ongoing"}
        </button>
        <Link
          to={"/work/1"}
          className="flex-1 py-2.5 text-center bg-teal-600 text-white rounded-lg text-sm font-medium"
        >
          Continue
        </Link>
      </div>
